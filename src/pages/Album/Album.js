    ...
    useEffect(() => {
        ...
        if (id) {
            async function loadData() {
                ...
                let artist;
                if (data) {
                    artist = await spotifyApi
                        .getArtist(data.artists[0].id)
                        .then((data) => data)
                        .catch((error) => {
                            console.log('Error', error);
                            if (error.status === 401) {
                        ...
                        <div className={cx('intro')}>
                            {resultData.artists.length > 1 ? (
                                ...
                                <div className={cx('header-creator-wrapper')}>
                                    ...
                                    <Link className={cx('header-creator')} to={`/artist/${artistData.id}`}>
                                        {artistData.name}
                                    </Link>
                                </div>
                            )}
                            ...
                            >
                                ...
                                <span className={cx('header-duration')}>
                                    {totalTime > 3599000 ? convertMsToHM(totalTime) : msToMinAndSeconds(totalTime)}
                                </span>
                            </div>
                        </div>
                    }
                        ...
                        <div className={cx('intro')}>
                            {resultData.artists.length > 1 ? (
                                ...
                                <div className={cx('header-creator-wrapper')}>
                                    ...
                                    <Link className={cx('header-creator')} to={`/user/${artistData.id}`}>
                                        {artistData.name}
                                    </Link>
                                </div>
                            )}
