...
function Playlist() {
    const { spotifyApi, setTokenError, msToMinAndSeconds, convertMsToHM, contextMenu, smallerWidth } =
        ...
        return smallerWidth ? (
                        ...
                        <div className={cx('playlist-intro')}>
                            <div className={cx('header-creator-wrapper')}>
                                {creatorPlaylist.images.length > 0 ? (
                                    <img
                                        loading="lazy"
                                        src={creatorPlaylist.images[0].url}
                                        alt={`Image of ${creatorPlaylist.name}`}
                                ...
                                <Link className={cx('header-creator')} to={`/user/${playlistData.owner.id} `}>
                            ...
                            >
                                <span className={cx('header-total')}>
                                    {playlistData.followers.total > 0 &&
                                        `${Intl.NumberFormat().format(playlistData.followers.total)} likes`}
                                </span>
                                <span className={cx('header-total')}>
                                    {playlistData.tracks.total > 0 &&
                                        `${playlistData.followers.total > 0 ? ' • ' : ''}${
                                            offset + 1
                                        } - ${displayMaxSong()}/${playlistData.tracks.total} songs, `}
                                </span>
                                {playlistData.tracks.total > 0 && (
            ...
            <PageContentLayout
                ...
                subTitle={
                    <>
                        {playlistData?.description !== '' && (
                            <div className={cx('header-sub-title')}>{playlistData?.description}</div>
                        )}
                        <div className={cx('playlist-intro')}>
                            <div className={cx('header-creator-wrapper')}>
                                {creatorPlaylist.images.length > 0 ? (
                            ...
                            <span className={cx('header-total')}>
                                {playlistData?.tracks.total > 0 &&
                                    ` • ${offset + 1} - ${displayMaxSong()}/${playlistData?.tracks.total} songs, `}
                            </span>
                            {playlistData?.tracks.total > 0 && (
                                <span className={cx('header-duration')}>
                                    about{' '}
            ...
            >
                {tracksData.items.length > 0 && (
                    <Segment
                        ...
                        isPlaylist
                        toPlaylistId={id}
                        titleForNextFrom={playlistData.name}
                        columnHeader
                        colHeaderIndex
                        colHeaderTitle
                        colHeaderAlbum
